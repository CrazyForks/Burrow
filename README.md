...
| Signed / notarized | Required for new official tags *(0.10.5 and earlier: no)* | No — unsigned preview | ✅ | ✅ | ✅ | n/a |
...
> New official macOS tags must pass Developer ID signing, Apple notarization,
> ticket stapling, and Gatekeeper assessment before the release can publish.
> Archived 0.10.5-and-earlier builds predate that gate and remain unsigned.
...
The archived 0.10.5 cask uses its legacy quarantine bypass. The release
workflow removes that bypass from the live tap only after publishing the first
verified Developer ID signed and notarized successor.
